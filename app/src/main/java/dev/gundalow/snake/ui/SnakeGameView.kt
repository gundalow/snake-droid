...
private fun AboutScreen(onClose: () -> Unit) {
    Surface(modifier = Modifier.fillMaxSize(), color = Color.Black.copy(alpha = 0.95f)) {
        Column(
            ...
            Text("About Snake", style = MaterialTheme.typography.headlineMedium, color = Color.White)
            Spacer(modifier = Modifier.height(16.dp))
            Text("Repository:", color = Color.Gray)
            Text("github.com/gundalow/snake", color = Color.Cyan)
            Spacer(modifier = Modifier.height(16.dp))
            Text("Build Date:", color = Color.Gray)
            Text("2025-05-20", color = Color.White) // Static build date as requested
